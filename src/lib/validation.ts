...
function checkSources(c: OSCECase): CheckResult {
  ...
  return {
    ...
    detail:
      "Needs case-specific source tagging. Default Queensland Health protocol fallback is shown only for display continuity.",
  };
}
...
function checkDiagnosisConsistency(c: OSCECase): CheckResult {
  ...
  const corpus = joinLC(
    c.candidateStem,
    c.condition,
    c.fakePatientScript?.openingLine,
    c.fakePatientScript?.mainComplaint,
    c.fakePatientScript?.socrates,
  ...
  const patterns: Array<{ dxIncludes: string[]; expect: string[]; reason: string }> = [
    {
      dxIncludes: ["otitis externa", "swimmer"],
      expect: ["pinna", "tragus", "canal", "swim", "water"],
      reason:
        "Otitis externa cases usually mention tragus/pinna tenderness, canal involvement or water exposure.",
    },
    {
      dxIncludes: ["otitis media", "aom"],
      expect: ["ear pain", "earache", "tm", "tympanic", "fever", "child", "uri"],
      reason: "AOM cases usually mention ear pain plus URI features or TM findings.",
    },
    {
      dxIncludes: ["allergic rhinitis"],
      expect: ["sneez", "itch", "watery", "bilateral", "season", "pollen", "dust"],
      reason: "Allergic rhinitis should fit sneeze/itch/watery/bilateral/seasonal pattern.",
    },
    {
      dxIncludes: ["gord", "reflux", "heartburn"],
      expect: [
        "heartburn",
        "reflux",
        "retrosternal",
      ...
      reason: "GORD should fit reflux/retrosternal/postprandial pattern.",
    },
    {
      dxIncludes: ["asthma"],
      expect: ["wheeze", "reliever", "salbutamol", "cough", "short of breath", "sob"],
      reason: "Asthma cases should reference wheeze, reliever use or breathlessness.",
    },
    {
      dxIncludes: ["copd"],
      expect: ["smok", "sputum", "wheeze", "breathless", "exacerbation"],
      reason: "COPD cases should reference smoking history, sputum or breathlessness pattern.",
    },
    {
      dxIncludes: ["gastroenteritis", "food poison"],
      expect: ["diarrhoea", "vomit", "nausea", "abdominal", "loose"],
      reason: "Gastroenteritis cases should fit diarrhoea/vomiting/nausea pattern.",
    },
    {
      dxIncludes: ["impetigo"],
      expect: ["crust", "honey", "vesic", "lesion", "skin", "rash"],
      reason: "Impetigo cases should reference honey-coloured crusting or vesicular lesions.",
    },
    {
      dxIncludes: ["uti", "cystitis"],
      expect: ["dysuria", "frequency", "urgency", "burning", "urine"],
      reason: "UTI cases should fit dysuria/frequency/urgency pattern.",
    },
  ];
...
function checkExamConsistency(c: OSCECase): CheckResult {
  ...
  const rules: Array<{ dx: string[]; expect: string[]; reason: string }> = [
    {
      dx: ["otitis externa"],
      expect: ["tragus", "pinna", "canal", "discharge"],
      reason: "Otitis externa exam should reference tragus/pinna pain or external canal findings.",
    },
    {
      dx: ["otitis media", "aom"],
      expect: ["tympanic", "tm ", "membrane", "bulg"],
      reason: "AOM exam should reference tympanic membrane appearance.",
    },
    {
      dx: ["allergic rhinitis"],
      expect: ["turbinate", "conjunct", "rhinorr", "nasal", "shiner"],
      reason:
        "Allergic rhinitis exam should reference nasal turbinates or conjunctival/ocular findings.",
    },
    {
      dx: ["asthma", "copd"],
      expect: ["wheeze", "auscult", "spo", "rr", "crackle"],
      reason: "Respiratory cases should reference auscultation, wheeze, RR or SpO₂.",
    },
    {
      dx: ["gord", "reflux", "heartburn"],
      expect: ["epigastr", "abdomen", "tender", "soft"],
      reason: "GORD exam should reference abdominal/epigastric assessment.",
    },
    {
      dx: ["gastroenteritis"],
      expect: ["abdomen", "bowel", "tender", "hydration", "mucous"],
      reason: "Gastroenteritis exam should reference abdomen, bowel sounds or hydration.",
    },
    {
      dx: ["impetigo"],
      expect: ["lesion", "crust", "vesic", "lymph"],
      reason: "Impetigo exam should describe lesions, crusting or lymph nodes.",
    },
  ];
...
function checkVitalsSeverity(c: OSCECase): CheckResult {
  if (!c.vitals || Object.keys(c.vitals).length === 0) {
    return {
      id: "vitals-plausible",
      label: "Vitals are plausible",
      level: "warning",
      detail: "No vitals recorded.",
    };
  }
  const v = c.vitals;
  const key = (re: RegExp) => {
...
function checkRedFlagReferral(c: OSCECase): CheckResult {
  ...
  if (
    hasEmergencyPresent &&
    c.scopeDecision !== "emergency" &&
    c.referralClassification !== "emergency-referral" &&
    c.referralClassification !== "urgent-referral"
  ) {
    return {
      id: "redflag-coverage",
      label: "Red flags are checked",
...
function checkReferralJustified(c: OSCECase): CheckResult {
  const refer =
    c.scopeDecision === "treat-and-refer" ||
    c.scopeDecision === "refer-only" ||
    c.scopeDecision === "emergency";
  if (refer) {
    const reason = (c.referralPlan ?? "").trim();
    if (reason.length < 20) {
      return {
        id: "referral-justified",
        label: "Referral decision is justified",
        level: "warning",
        detail: "Referral plan is missing or too short to be specific.",
      };
    }
  }
...
function checkScope(c: OSCECase): CheckResult {
  if (!c.scopeDecision) {
    return {
      id: "scope-decision",
      label: "Treatment is within pharmacist scope",
      level: "fail",
      critical: true,
      detail: "Missing scopeDecision.",
    };
  }
...
function checkDoseVerification(c: OSCECase): CheckResult {
  ...
  return {
    ...
    detail: hasVerify
      ? undefined
      : "treatmentPlanNotes does not mention verify/confirm against current protocol.",
  };
}
...
function checkPregnancyBF(c: OSCECase): CheckResult {
  ...
  if (!pt || pt.trim() === "") {
    return {
      id: "preg-bf",
      label: "Pregnancy/breastfeeding considered",
      level: "warning",
      detail: "pregnancyBreastfeeding field is empty.",
    };
  }
...
function checkAllergy(c: OSCECase): CheckResult {
  ...
  if (!al)
    return {
      id: "allergy",
      label: "Allergy considered",
      level: "warning",
      detail: "No allergies field on patient script.",
    };
  if (
    anyOf(al, ["penicillin", "amoxicillin", "augmentin"]) &&
    anyOf(al, ["anaphyl", "swelling", "throat"])
  ) {
...
function checkAgeConsiderations(c: OSCECase): CheckResult {
  ...
  if (typeof age !== "number") {
    return {
      id: "age-extremes",
      label: "Paediatric/older adult considerations checked",
      level: "warning",
      detail: "Patient age missing.",
    };
  }
  const reasoning = joinLC(c.protocolReasoning, c.clinicalReasoning, c.treatmentPlanNotes);
  if (
    (age < 16 || age >= 75) &&
    !anyOf(reasoning, ["age", "paediatric", "child", "elderly", "older adult", "geriatric"])
  ) {
    return {
      id: "age-extremes",
      label: "Paediatric/older adult considerations checked",
...
function checkCardiacMimic(c: OSCECase): CheckResult {
  ...
  const hasChestEpigastric = anyOf(corpus, [
    "chest pain",
    "retrosternal",
    "epigastr",
    "chest tight",
  ...
  const considers = anyOf(reasoning, [
    "cardiac",
    "acs",
    "angina",
    "ischaem",
  ...
  if (hasHeartburnOnly && !hasChestEpigastric && !hasAtypical) {
    return {
      id: "cardiac-mimic",
      label: "Cardiac mimics not missed",
      level: "warning",
      detail:
        "Heartburn/indigestion presentation: consider adding cardiac mimic (ACS, angina) to differentials or screening questions.",
    };
  }
  return {
    id: "cardiac-mimic",
    ...
    critical: true,
    detail:
      "Chest/epigastric or atypical pain features present but cardiac mimic (ACS, angina) is not considered in reasoning, differentials or critical fails.",
  };
}
...
function checkSafetyNetSpecific(c: OSCECase): CheckResult {
  ...
  if (sn.length === 0) {
    return {
      id: "safetynet-specific",
      label: "Safety-netting is specific",
      level: "warning",
      detail: "No safety-net entries.",
    };
  }
  const generic = sn.some(
    (s) =>
      /seek (medical )?attention if symptoms worsen|see (your )?gp if symptoms persist/i.test(s) &&
      s.length < 80,
  );
  if (generic && sn.length === 1) {
    return {
      ...
      label: "Safety-netting is specific",
      level: "warning",
      detail: "Safety-net is generic. Name specific symptoms, timeframes, and where to go.",
    };
  }
...
function checkGPLetter(c: OSCECase): CheckResult {
  ...
  if (needsRefer && !hasSbar && !hasLetter) {
    return {
      id: "gp-letter",
      label: "GP letter is clinically useful",
      level: "warning",
      detail: "Referral case has no sbar/gpLetter content.",
    };
  }
...
function checkCustomViva(c: OSCECase): CheckResult {
  ...
  if (n === 0)
    return {
      id: "viva-custom",
      label: "Custom viva block exists",
      level: "warning",
      detail: "Missing custom viva block. Viva Mode will fall back to generic questions.",
    };
  return {
    id: "viva-custom",
    label: "Custom viva block exists",
    level: "warning",
    detail: `Only ${n} custom viva question(s); generic fillers will be used.`,
  };
}
...
function checkBriefLeak(c: OSCECase): CheckResult {
  ...
  if (!brief) {
    return {
      id: "brief-leak",
      label: "Hidden findings do not leak into Student Brief",
      level: "warning",
      detail: "candidateStem is empty.",
    };
  }
  ...
  for (const h of hiddenStrings) {
    ...
    if (snippet.length >= 20 && brief.includes(snippet.slice(0, 25))) {
      return {
        ...
        critical: true,
        detail:
          "An examiner-only finding (hidden answer, examination finding, or vitals reading) appears in the student brief.",
      };
    }
  }
  ...
  const referralLeak =
    /\b(refer to (gp|ed)|emergency referral|prescribe \w+|administer \w+)\b/i.test(c.candidateStem);
  if (referralLeak) {
    return {
      id: "brief-leak",
      label: "Hidden findings do not leak into Student Brief",
      ...
      critical: true,
      detail:
        "Student brief reveals management/referral decision that should remain examiner-only.",
    };
  }
...
function checkEmDashes(c: OSCECase): CheckResult {
  ...
  if (corpus.includes("—") || corpus.includes("–")) {
    return {
      ...
      label: "No em dashes",
      level: "warning",
      detail: "Em or en dash found in case content.",
    };
  }
...
function checkAIFiller(c: OSCECase): CheckResult {
  ...
  return {
    id: "ai-filler",
    label: "No AI-sounding filler",
    level: "warning",
    detail: `Found: ${hits.join(", ")}`,
  };
}
...
function checkUnsupportedClaims(c: OSCECase): CheckResult {
  ...
  if (
    longestSentence.length > 220 &&
    !/protocol|verify|reference|guideline/i.test(longestSentence)
  ) {
    return {
      id: "unsupported-claims",
      label: "No unsupported claims",
      level: "warning",
      detail: "Long protocolReasoning sentence with no protocol/guideline anchor.",
    };
  }
...
function checkReferralClassification(c: OSCECase): CheckResult {
  ...
  if (!c.referralClassification) {
    return {
      id: "referral-class",
      label: "Referral classification set",
      level: "warning",
      detail: "No referralClassification (one of: " + allowed.join(", ") + ").",
    };
  }
