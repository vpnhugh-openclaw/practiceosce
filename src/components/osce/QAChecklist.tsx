...
export function QAChecklist({ case: c, defaultOpen = false, compact = false }: Props) {
  ...
  const summary = (
    <span className={`inline-flex items-center gap-1.5 rounded-md border px-2 py-1 text-xs ${LEVEL_CLASS[report.overall]}`}>
      {report.overall === "fail" ? <ShieldAlert className="h-3.5 w-3.5" /> :
        report.overall === "pass" ? <ShieldCheck className="h-3.5 w-3.5" /> :
        ...
        <div className="px-3 pb-3 space-y-1.5">
          {report.unsafeForTimedPractice && (
            <p className="text-xs text-red-900 bg-red-100 border border-red-300 rounded px-2 py-1.5">
              <strong>Unsafe for timed practice.</strong> Critical check failed: case can only be opened in editing/debug mode.
            </p>
          )}
          {ORDER.flatMap((lvl) =>
            report.results
              ...
              .map((r) => (
                <div key={r.id} className={`text-xs rounded border px-2 py-1.5 ${LEVEL_CLASS[r.level]}`}>
                  <p className="font-medium">
                    [{LEVEL_LABEL[r.level]}] {r.label}
                    {r.critical && r.level === "fail" && <span className="ml-1 uppercase text-[10px]">critical</span>}
                  </p>
