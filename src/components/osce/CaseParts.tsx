...
import { ScopeDecisionBadge, ProtocolConfidenceBadge, ProtocolCheckWarning, RedFlagBox } from "./ScopeBadges";
  ...
  const cols: { title: string; rows: string[] }[] = [
    {
      ...
      rows: ["Presenting complaint", "Focused questions (SOCRATES)", "Red flags to screen", "Hidden answers"],
    },
    {
      title: "SNAP / Vitals / Examination",
      rows: ["SNAP lifestyle", "Vitals to request", "Examination to perform", "Findings to interpret"],
    },
    {
      title: "PMHx / Allergies / Family / Vaccinations",
      rows: ["Medical history", "Medications & allergies", "Pregnancy / breastfeeding", "Family & vaccination history"],
    },
  ];
  return (
    <div className="grid md:grid-cols-3 gap-4">
      {cols.map((col) => (
        <div key={col.title} className="handbook-card p-4 bg-parchment/60">
          <p className="text-[11px] uppercase tracking-wide text-muted-foreground mb-2">{col.title}</p>
  ...
  return (
    <div className="handbook-card p-5">
      ...
      <div className="mt-4 grid sm:grid-cols-2 gap-3">
        <div>
          <p className="text-[11px] uppercase tracking-wide text-muted-foreground mb-1">Working diagnosis</p>
          <p className="text-sm font-medium">{c.expectedDiagnosis}</p>
        </div>
        <div>
          <p className="text-[11px] uppercase tracking-wide text-muted-foreground mb-1">Differentials</p>
          <ul className="text-sm">
            {c.differentials.map((d) => <li key={d}>• {d}</li>)}
          </ul>
        </div>
      </div>
    </div>
  ...
  return (
    <Section title="Non-pharmacological plan">
      <ul className="text-sm space-y-1.5">
        {c.nonPharmPlan.map((t) => <li key={t}>• {t}</li>)}
      </ul>
    </Section>
  );
}
  ...
  return (
    <Section title="Safety net & follow-up">
      ...
      <ul className="text-sm space-y-1.5 mb-3">
        {c.safetyNet.map((s) => <li key={s}>• {s}</li>)}
      </ul>
      {c.referralPlan && (
