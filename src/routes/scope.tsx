...
function ScopePage() {
  ...
  return (
    <div>
      <PageHeader eyebrow="Scope Checker" title="Quick scope decision." subtitle="Enter the key risk inputs to get a protocol-aligned scope decision and suggested OSCE wording." />
      ...
      <div className="grid lg:grid-cols-[1fr_1fr] gap-5">
        <div className="handbook-card p-5 space-y-4">
          <label className="block">
            ...
            <select value={conditionId} onChange={(e) => setConditionId(e.target.value)} className="mt-1 w-full rounded-md border border-input bg-card px-3 py-2 text-sm">
          ...
          <label className="block">
            ...
            <input type="number" value={age} onChange={(e) => setAge(Number(e.target.value))} className="mt-1 w-full rounded-md border border-input bg-card px-3 py-2 text-sm" />
          </label>
          ...
          <Toggle label="Marked emotional / functional impact" v={emotionalImpact} on={setEmotionalImpact} />
          {p.previousDiagnosisRequired && <Toggle label="Previous diagnosis on file" v={previousDx} on={setPreviousDx} />}
        </div>
        ...
        <div className="space-y-4">
          ...
          <Section title="Protocol summary — key criteria">
            <div className="text-sm space-y-2">
              ...
              <p><strong>Out of scope:</strong> {p.outOfScopeCriteria.join("; ")}</p>
              <p><strong>Review:</strong> {p.reviewTime}</p>
            </div>
          </Section>
        </div>
      </div>
