    ...
    <div>
      ...
      <div className="handbook-card p-5 mb-5 grid md:grid-cols-3 gap-4">
        <label className="block">
          ...
          <select value={id} onChange={(e) => setId(e.target.value)} className="mt-1 w-full rounded-md border border-input bg-card px-3 py-2 text-sm">
            {PROTOCOLS.map((p) => <option key={p.id} value={p.id}>{p.conditionName}</option>)}
          </select>
        </label>
        <label className="block">
          <span className="text-[11px] uppercase tracking-wide text-muted-foreground">Patient name</span>
          <input value={patientName} onChange={(e) => setPatientName(e.target.value)} className="mt-1 w-full rounded-md border border-input bg-card px-3 py-2 text-sm" />
        </label>
        <label className="flex items-center gap-2 text-sm pt-5">
          <input type="checkbox" checked={refer} onChange={(e) => setRefer(e.target.checked)} /> GP referral required
        </label>
      </div>
