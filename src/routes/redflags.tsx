...
function RedFlagsPage() {
  ...
  const filtered = RED_FLAGS.filter((r) =>
    [r.flag, r.suggests, r.action, ...(r.appliesTo ?? []), r.acronym ?? ""]
      .join(" ")
      .toLowerCase()
  ...
  return (
    <div>
      <PageHeader
        eyebrow="Red Flag Library"
        title="Acronyms, alarms, and what to say."
        subtitle="Searchable red flags drawn from Queensland-protocol-aligned pharmacist prescribing OSCE practice."
      />
          ...
          {ACRONYMS.map((a) => (
            <div key={a.name} className="rounded-md border border-border p-3 bg-parchment/60">
              ...
              <ul className="text-sm space-y-0.5">
                {a.items.map((i, idx) => (
                  <li key={idx}>
                    <span className="font-mono text-navy">{i.letter}</span>: {i.flag}
                  </li>
                ))}
              </ul>
            </div>
        ...
        {filtered.map((r, i) => (
          <div key={i} className="handbook-card p-4">
            <div className="flex items-center gap-2 flex-wrap mb-1">
              {r.acronym && (
                <span className="text-[10px] uppercase tracking-wide bg-navy text-navy-foreground px-1.5 py-0.5 rounded">
                  {r.acronym}
                </span>
              )}
            ...
            <p className="text-sm mt-1">
              <strong>Suggests:</strong> {r.suggests}
            </p>
            <p className="text-sm">
              <strong>Action:</strong> {r.action}
            </p>
            {r.oscePhrase && (
              <p className="text-sm italic mt-1 text-muted-foreground">"{r.oscePhrase}"</p>
            )}
          </div>
        ))}
      </div>
