...
function ProtocolsPage() {
  return (
    <div>
      <PageHeader
        eyebrow="Condition Protocol Cards"
        title="Quick clinical reference."
        subtitle="Age limits, in/out-of-scope criteria, red flags, review times and communication frameworks per condition. Always verify the latest Queensland Health clinical practice guideline before prescribing."
      />
      <div className="space-y-4">
        {PROTOCOLS.map((p) => (
          <Section
            key={p.id}
            title={p.conditionName}
            badge={<ProtocolConfidenceBadge confidence={p.protocolConfidence} />}
            defaultOpen={false}
          >
            <div className="grid md:grid-cols-2 gap-3 text-sm">
              ...
              <div>
                <p className="text-[11px] uppercase tracking-wide text-muted-foreground mb-1">
                  Treat & refer triggers
                </p>
                <ul className="mb-2">
                  {p.treatAndReferCriteria.map((s) => (
                    <li key={s}>• {s}</li>
                  ))}
                </ul>
                ...
                <ul>
                  {p.emergencyReferralCriteria.map((s) => (
                    <li key={s}>• {s}</li>
                  ))}
                </ul>
              </div>
              ...
              <div className="md:col-span-2">
                ...
                <ul className="mb-2 columns-2">
                  {p.nonPharmacologicalAdvice.map((s) => (
                    <li key={s}>• {s}</li>
                  ))}
                </ul>
                <p className="text-[11px] uppercase tracking-wide text-muted-foreground mb-1">
                  Pharmacological options (class only: verify dose)
                </p>
                <ul className="mb-2">
                  {p.pharmacologicalOptions.map((s) => (
                    <li key={s}>• {s}</li>
                  ))}
                </ul>
                <p className="text-[11px] uppercase tracking-wide text-muted-foreground mb-1">
            ...
            <p className="text-xs text-muted-foreground italic mt-3">
              Source: {p.protocolSource} · Version: {p.protocolVersion}
            </p>
            <PageSourcesDrawer
